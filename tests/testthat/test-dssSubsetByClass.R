test_that("dssSubsetByClass works", {
  ...
  dssSubsetByClass('iris', variables = c('iris$Species', 'b'), subsets = 'with_b', datasources =  opals['local2'])
  ...
  dssSubsetByClass(c('iris', 'a'), variables = c('iris$Species', 'b'), subsets = 'with_ab',
                   keep.cols = c('Sepal.Length', 'Sepal.Width', 'a'), datasources = opals['local2'])
  expect_equal(mean(play_env$with_ab$setosa.category1$a), 1.47)
  dssSubsetByClass('a', variables = 'b',subsets = 'a_and_b',keep.cols = c('a'), datasources = opals['local2'])
